...
if __name__ == "__main__":
    ...
    general_product = load_decay_rates(
        dataset=dataset, kernel="GeneralProduct", id=id
    )
    jenga = load_decay_rates(dataset=dataset, kernel="Jenga", id=id)
    jenga = get_jenga_mut_decay_rates(jenga)
    dfs = {"Jenga": jenga, "General Product": general_product}

    # Mutation specific decay factors
    figsize = (0.8 * FIG_WIDTH, 1.4 * FIG_WIDTH)
    ...
    fig.savefig("figures/gb1.decay_factors_supp.png", dpi=300)
    fig.savefig("figures/gb1.decay_factors_supp.svg", dpi=300)
