...
def read_decay_factors(dataset, kernel="Connectedness"):
    ...
    logit_rho = params["covar_module.module.logit_rho"].numpy()
    log_p = params["covar_module.module.log_p"].numpy()
    rho = np.exp(logit_rho) / (1 + np.exp(logit_rho))
    p = np.exp(log_p)
    p = p / np.expand_dims(p.sum(1), 1)
    eta = (1 - p) / p
    decay_factor = (1 - (1 - rho) / (1 + eta * rho)).mean(1) * 100
    return decay_factor
...
if __name__ == "__main__":
    ...
    sel_loci["decay_rate"] = read_decay_factors(dataset)
    sel_loci["idx"] = np.arange(sel_loci.shape[0])
    sel_loci.to_csv("results/qtls_li_hq_annotations.csv")
    print(sel_loci)
    ...
    axes.set(
        title="Growth rate in {}".format(environment.capitalize()),
    ...
    np.random.seed(21)
    labels = (
        sel_loci.dropna(subset=["gene"])
        .sort_values("decay_rate", ascending=False)
        .iloc[:10, :]
    )
    ...
    fig.tight_layout()
    fig.savefig("plots/{}.decay_factors.png".format(env_label), dpi=300)
    fig.savefig("plots/{}.decay_factors.pdf".format(env_label), dpi=300)
