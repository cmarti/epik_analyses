...
if __name__ == '__main__':
    ...
    decay_rates = read_decay_factors('qtls_merged_hq', id=id).mean(1).values * 100
    print(decay_rates)
    env_decay_rate = decay_rates[-1]
    sel_loci['decay_rate'] = decay_rates[:-1]
    ...
    np.random.seed(21)
    labels = sel_loci.dropna(subset=['gene']).sort_values('decay_rate', ascending=False).iloc[:4, :]
    dxs = [-1, 1, 1, -1, -1, -1, -0.5, 1, 1.5, -1]
    dys = [0, 0.25, 0.35, 0, 1, 0, 2, 2, 1, 0]
    for x, y, label, dx, dy in zip(labels['x'], labels['decay_rate'], labels['gene'], dxs, dys):
        xtext = x + 2e5 * dx
        ytext = y + 0.25 +  dy
        ha = 'left' if xtext > x else 'right'
        axes.annotate(label, xy=(x, y+0.05), ha=ha,
                      xytext=(xtext, ytext), fontsize=7,
                      arrowprops=dict(facecolor='black', shrink=1,
                                      width=0.25, headwidth=2, headlength=2))
    axes.axhline(env_decay_rate, lw=0.75, linestyle='--', c='purple', label='Environment')
    axes.legend(loc=(0.25, 1.025), frameon=True, ncol=2)
