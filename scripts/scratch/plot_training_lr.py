...
if __name__ == "__main__":
    ...
    for dataset, axes in zip(['smn1', 'aav', 'gb1'], subplots):
        ...
        for lr, c in zip(lrs, colors):
            for i in range(1, 4):
                fpath = "output/{}.Adam.opt_lr.{}.{}.loss.csv".format(dataset, lr, i)
                if not exists(fpath):
                    fpath = "output/{}.opt_lr.{}.{}.loss.csv".format(dataset, lr, i)
                    if not exists(fpath):
                        # print(fpath)
                        continue
    ...
    fig.savefig("plots/learning_rates.png", format="png", dpi=300)
    print("Done")
