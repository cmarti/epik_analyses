...
from gpmap.src.linop import calc_covariance_vjs, calc_covariance_distance
...
if __name__ == '__main__':
    for dataset in ['smn1', 'gb1']:
        ...
        a, l, alphabet = config['n_alleles'][0], config['length'], np.unique(np.hstack(config['alphabet']))
        ...
        prior_cov = [np.prod([1 + (a - 1) * r if i == 0 else 1 - r for i, r in zip(sites, rho)])
