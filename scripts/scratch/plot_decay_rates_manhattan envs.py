...
if __name__ == '__main__':
    ...
    sel_loci['decay_rate_li'] = read_decay_factors('qtls_li', id=id).mean(1).values * 100
    sel_loci['decay_rate_30C'] = read_decay_factors('qtls_30C_hq', id=id).mean(1).values * 100

    fig, axes = plt.subplots(1, 1, figsize=(6.5, 3))
    axes.scatter(sel_loci['x'], sel_loci['decay_rate_li'], c='purple', s=7.5, lw=0, label='$Li^+$', alpha=0.5)
    axes.scatter(sel_loci['x'], sel_loci['decay_rate_30C'], c='darkorange', s=7.5, lw=0, label='30ºC', alpha=0.5)
    axes.legend(loc=2)
    ...
    axes.set(xlabel='Chromomsome position (bp)', ylabel='Decay factor (%)',
             ylim=ylim, xlim=(0, chr_x.max()),
             xticks=chr_pos)
    axes.set_xticklabels(chr_labels, rotation=45)

    np.random.seed(21)
    labels = sel_loci.dropna(subset=['gene']).sort_values('decay_rate_li', ascending=False).iloc[:10, :]
    dxs = [-1, 1, 1, 2, -1, -1, -0.5, 1, 1.5, -1]
    dys = [0, 1, 0.7, 0, 1, 0, 2, 2, 1, 0]
    for x, y, label, dx, dy in zip(labels['x'], labels['decay_rate_li'], labels['gene'], dxs, dys):
        xtext = x + 2e5 * dx
        ytext = y + 1 +  dy
    ...
    fig.tight_layout()
    fig.savefig('plots/envs.decay_factors.png'.format(environment), dpi=300)
    fig.savefig('plots/envs.decay_factors.pdf'.format(environment), dpi=300)
